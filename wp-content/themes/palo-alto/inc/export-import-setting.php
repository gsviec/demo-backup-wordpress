...
function wpkube_import_setting(){
	...
	if ( isset($_POST['submit_import']) && isset($_FILES['import'])){
		...
		if ( ($import =  wp_remote_get($_FILES['import']['tmp_name'] )) !== false ){
			...
			if ($getsomething){
				$msg = __('Import setting success', 'palo-alto');
			}else{
				$msg = __('Nothing imported', 'palo-alto');
			}
		}else{
			$msg = __('Nothing imported', 'palo-alto');
		}
...
<div class="wrap export-import">
	<h2><?php _e('Export and Import Setting', 'palo-alto'); ?></h2>
	<form enctype="multipart/form-data" method="post">
    	<!--<h3>Reset Setting to Default</h3>
        <p><input name="submit_reset" id="submit" class="button" value="<?php _e('Reset setting to default', 'palo-alto'); ?>" type="submit"></p>
        <p class="description"><?php _e('This operation will reset all the setting to the default state. Please note that once you reset there is no undo, it would be better if you backup your current setting first by exporting it.','palo-alto'); ?></p>
        <br />-->
		<h3>Import Setting</h3>
    	<p><?php _e('Upload a valid .json file and we will import the theme settings into this theme', 'palo-alto'); ?></p>
        <p><?php _e('Choose a .json file to upload, then click Upload file and import.', 'palo-alto'); ?></p>
        <p><input id="upload" name="import" size="25" type="file"></p>
        <p><input name="submit_import" id="submit" class="button" value="<?php _e('Upload file and import', 'palo-alto'); ?>" type="submit"></p>
        <p class="description"><?php _e('This operation will replace the current setting to the uploaded one. Please note that once you import there is no undo, it would be better if you backup your current setting first by exporting it.','palo-alto'); ?></p>
        <br />
        <h3>Export Current Setting</h3>
        <p><input name="submit_export" id="submit_export" class="button" value="<?php _e('Export current setting','palo-alto'); ?>" type="submit"></p>
        <p><?php _e('This will download the setting in JSON format', 'palo-alto'); ?></p>
    </form>
